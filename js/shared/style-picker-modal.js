 ...
 * - /js/shared/add-to-collection-modal.js (for collection functionality)
...
function StylePickerModal({ 
  ...
  // Updated: Add to Collection handler - now opens the modal
  const handleAddToCollectionClick = (styleId, styleName, e) => {
    ...
    // Otherwise, open the Add to Collection modal
    if (window.SocietyArts?.openAddToCollectionModal) {
      window.SocietyArts.openAddToCollectionModal(styleId, styleName);
    } else {
      console.warn('[StylePickerModal] Add to Collection modal not available. Include /js/shared/add-to-collection-modal.js');
    }
  };
